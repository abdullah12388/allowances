$(function () {
    $.ajax({
        url: "/tank/alarm/current/table/data/api/",
        method: "GET",
        dataType: "json",
        success: function (t) {
            // console.log(t);
...
function filterData(filters, filtered, data){
    ...
    if(filters.datefrom !== ""){
        ...
        const [hours, minutes] = filters.datefrom.split(":");
        currentDate.setHours(hours);
        currentDate.setMinutes(minutes);
        // console.log(currentDate);
        // var formatedDate = formatDate(currentDate);
        originalData = originalData.filter((obj) => obj.time >= currentDate.toISOString())
        // console.log(originalData);
        // console.log(filters.datefrom, currentDate, originalData);
    }
    if(filters.dateto !== ""){
        const currentDate = new Date();
        const [hours, minutes] = filters.dateto.split(":");
        currentDate.setHours(hours);
        currentDate.setMinutes(minutes);
        // console.log(currentDate);
        // var formatedDate = formatDate(currentDate);
...
document.getElementById("print_selected").addEventListener("click", function () {
    var selectedRows = [];
    selectedRows = $("#currentAlarmsTbl").bootstrapTable("getSelections");
    console.log(selectedRows);
    var printWindow = window.open("", "_blank");
    var currentDate = new Date();
    printWindow.document.open();
    printWindow.document.write(
        "<html><head><title>Current Alarms Table</title>" +
        "<style>" +
    ...
    var columnNames = [];
    $("#currentAlarmsTbl thead th").each(function () {
