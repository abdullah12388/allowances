$(function () {
    ...
    $.ajax({
        url: "/tank/delivery/history/table/data/api/"+pageNumber+"/",
        method: "GET",
        dataType: "json",
        success: function (t) {
            console.log(t);
...
function filterData(filters, filtered, data){
    ...
    if(filters.datefrom !== ""){
        originalData = originalData.filter((obj) => obj.dateend >= filters.datefrom)
        console.log(originalData);
    }
    if(filters.dateto !== ""){
        originalData = originalData.filter((obj) => obj.dateend <= filters.dateto)
        console.log(originalData);
    }
    return originalData;
}
...
$("#toolbar")
    ...
    .change(function () {
        ...
        o.bootstrapTable("destroy").bootstrapTable({
            ...
            columns: [
                { field: "id", title: "ID" },
                { field: "datestart", title: "Date Start" },
                { field: "volstart", title: "Vol. Start" },
                { field: "voltotal", title: "Vol. Total" },
                { field: "volend", title: "Vol. End" },
                { field: "dateend", title: "Date End" },
...
document.getElementById("print_selected").addEventListener("click", function () {
    var selectedRows = [];
    selectedRows = $("#historyDeliveryTbl").bootstrapTable("getSelections");
    console.log(selectedRows);
    var printWindow = window.open("", "_blank");
    var currentDate = new Date();
    printWindow.document.open();
    printWindow.document.write(
        "<html><head><title>Historical Deliveries Table</title>" +
        "<style>" +
    ...
    var columnNames = [];
    $("#historyDeliveryTbl thead th").each(function () {
